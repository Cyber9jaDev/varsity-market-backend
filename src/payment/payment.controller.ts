...
export class PaymentController {
  ...
  @ApiBody({
    required: true,
    description: 'Initiate Paystack Transaction',
    type: TransactionInitializationDto,
  })
  async initializeTransaction(
    ...
    if (body.quantity > product.quantity)
      throw new BadRequestException(
        `Only ${product.quantity} items available for sale`,
      );
    ...
    return await this.paymentService.initializeTransaction(
      buyer.email,
      body.quantity,
      product.price,
      product.seller.subaccountCode,
      reference,
      body.callback_url,
    );
  }
  ...
  async verifyTransaction(@Param('reference') reference: string) {
    ...
    if (existingTransaction.status === PaymentStatus.SUCCESS) {
      return {
        ...existingTransaction,
        total: existingTransaction.amount * existingTransaction.quantity,
      };
    }
      ...
      try {
        return await this.databaseService.$transaction(async (db) => {
          ...
          return {
            ...
            total:
              successfulTransaction.amount * successfulTransaction.quantity,
          };
        });
      } catch (error) {
